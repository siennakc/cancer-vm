"""v4 head: fc import + prior + temperature on the HR embedding cache (splits_v2)."""
import json, sys
from pathlib import Path
import numpy as np
sys.path.insert(0, "src")
import torch
from oncoscope.data.mammography import read_case_table
from oncoscope.data.splits import load_manifest
from oncoscope.eval.metrics import auroc
from oncoscope.models.head import LogisticHead

EMB = Path("data/embeddings/resnet50_ft_v5hr_1152x896_raw")
cases = read_case_table("data/processed/cases_v1.jsonl")
splits = load_manifest("data/processed/splits_v2.json")
d = {}
for c in cases:
    s = splits.split_of(f"{c.site}/{c.patient_id}")
    if s == "public_bench":
        continue
    e = d.setdefault(s, {"X": [], "y": []})
    e["X"].append(np.load(EMB / f"{c.case_id}.npy")); e["y"].append(c.label)
d = {k: {"X": np.stack(v["X"]), "y": np.array(v["y"], float)} for k, v in d.items()}

ck = torch.load("runs/posttrain_v5hr/best_model.pt", map_location="cpu", weights_only=False)
head = LogisticHead(weights=ck["model"]["fc.weight"].numpy().ravel().astype(np.float64),
                    bias=float(ck["model"]["fc.bias"].numpy()[0]))
shift = head.apply_prior_correction(0.5, float(d["calibration"]["y"].mean()))
temp = head.calibrate_temperature(d["calibration"]["X"], d["calibration"]["y"])
p = head.predict_proba(d["threshold"]["X"])
neg = np.sort(p[d["threshold"]["y"] == 0])
tau = float(neg[min(max(int(np.ceil(0.96 * len(neg))), 1), len(neg)) - 1])
print(f"cal_auroc={auroc(d['calibration']['y'], head.predict_proba(d['calibration']['X'])):.4f} "
      f"shift={shift:.4f} temp={temp:.4f} tau={tau:.4f}")
out = Path("runs/finetune_v5hr_head"); out.mkdir(parents=True, exist_ok=True)
head.save(out / "head.json")
(out / "operating_point.json").write_text(json.dumps(
    {"threshold": tau, "target_specificity": 0.96}, indent=1))
